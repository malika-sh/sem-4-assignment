--  Create a trigger on product table for after update and back up the new data in new table 
...
AFTER UPDATE ON product111
FOR EACH ROW
BEGIN
    INSERT INTO product_new(prod_id,ptod_name, prod_amt, prod_comp, prod_mfgdate, prod_expdate)
    VALUES (:NEW.prod_id, :NEW.prod_name, :NEW.prod_amt, :NEW.prod_comp, :NEW.prod_mfgdate, :NEW.prod_expdate);
END;
/

UPDATE product SET prod_name='G15' WHERE prod_id='105';
